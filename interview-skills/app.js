...
function showPage(id,push=true){if(!pages.includes(id))id="home";document.querySelectorAll(".page").forEach(p=>p.classList.toggle("active",p.dataset.pagePanel===id));document.querySelectorAll(".course-nav button").forEach(b=>b.classList.toggle("active",b.dataset.page===id));const i=pages.indexOf(id),previousButton=document.getElementById("previousPage"),nextButton=document.getElementById("nextPage"),isFinal=i===pages.length-1;document.getElementById("pageCount").textContent=`${i+1} of ${pages.length}`;previousButton.disabled=i===0;previousButton.dataset.target=pages[Math.max(0,i-1)];nextButton.disabled=false;nextButton.dataset.target=isFinal?"academy-home":pages[i+1];nextButton.textContent=isFinal?"Course Complete →":"Next →";nextButton.setAttribute("aria-label",isFinal?"Course complete, return to Academy Home":"Go to the next course page");if(push)history.replaceState(null,"",`#${id}`);window.scrollTo({top:0,behavior:"smooth"})}
...
loadSaved();render();
document.querySelectorAll("[data-page]").forEach(b=>b.onclick=()=>showPage(b.dataset.page));document.querySelectorAll("[data-go]").forEach(b=>b.onclick=()=>showPage(b.dataset.go));document.getElementById("previousPage").onclick=e=>showPage(e.currentTarget.dataset.target);document.getElementById("nextPage").onclick=e=>{const target=e.currentTarget.dataset.target;if(target==="academy-home")window.location.href="../index.html";else showPage(target)};
